"""
用例 ID: 1150695810001062422
用例名称: 移动端设计器正常添加各营销模块
接口: POST /api/manage/designerSave/handle

流程：
1. 新增移动端自定义页面（addColInfo）
2. 获取页面详情（getColInfo）
3. 调用 designerSave/handle 保存营销模块配置
4. 再次获取页面详情，确认模块已保存
5. 删除自定义页面（清理数据）
"""
import json
...
def test_mobile_designer_marketing(admin_client):
    """移动端设计器：新增页面并添加营销模块。"""
    ...
    try:
        ...
        # 从 HAR 提取的营销模块完整配置
        yx_payload_path = Path(__file__).parent.parent.parent / "config/designer_payloads/mobile_marketing_modules.json"
        with open(yx_payload_path, encoding="utf-8") as f:
