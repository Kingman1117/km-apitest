...
2. 获取页面详情（getColInfo）
3. 调用 designerSave/handle（对齐最新抓包接口）
4. 保存页面数据（savePageData）
5. 再次获取页面详情，确认链路可执行
6. 删除自定义页面（清理数据）
"""
import json
...
def test_mobile_designer_common(admin_client):
    ...
    try:
        ...
        # 按 HAR 完整结构组装所有常用模块
        har_payload_path = Path(__file__).parent.parent.parent / "config/designer_payloads/mobile_common_modules.json"
        with open(har_payload_path, encoding="utf-8") as f:
