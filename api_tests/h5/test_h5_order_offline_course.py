"""
用例 ID: 1150695810001062382
用例名称: H5端正常提交线下课订单并管理后台退款
...
def test_h5_order_offline_course_and_refund(h5_client, admin_client):
    """H5端线下课订单 - 下单→查单→退款完整流程（复杂itemList）"""
    ...
    item_list = TestDataManager.get_order_item_template("h5_offline_course")
    item_list[0]["stuId"] = int(stu_id)

    # Act: H5端提交线下课订单（特殊itemList结构）
    ...
    logger.info("H5 线下课订单提交成功: orderNo=%s", order_no)
