"""
用例 ID: 1150695810001062381
用例名称: H5端正常提交课外服务订单并管理后台退款
...
def test_h5_order_book_service_and_refund(h5_client, admin_client):
    """H5端课外服务订单 - 下单→查单→退款完整流程（复杂itemList）"""
    ...
    item_list = TestDataManager.get_order_item_template("h5_book_service")
    item_list[0]["stuId"] = int(stu_id)

    # Act: H5端提交课外服务订单（特殊itemList结构）
